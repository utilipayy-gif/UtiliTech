...
export function UtiliHeader({services}:{services:Service[]}){
  ...
  return <header className="nsx-header" id="top">
    <Link className="brand" href="/"><Image className="brand-logo" src="/logo-mark.svg" alt="" width={34} height={34} priority/><span>UTILIPAY HUB</span></Link>
...
export async function UtiliFooter(){const settings=await getSiteSettings();return <><footer className="nsx-footer"><div><Link className="brand" href="/"><Image className="brand-logo" src="/logo-mark.svg" alt="" width={34} height={34}/><span>UTILIPAY HUB</span></Link><p>Web, application and digital growth services for ambitious teams.</p><small>{settings.legalName}</small><small>{settings.address}</small></div><div><b>EXPLORE</b><Link href="/services">Services</Link><Link href="/checkout">Order services</Link><Link href="/about">About us</Link><Link href="/contact">Contact</Link></div><div><b>LEGAL</b><Link href="/privacy">Privacy</Link><Link href="/refund">Refund &amp; cancellation</Link><Link href="/terms">Terms &amp; conditions</Link><Link href="/shipping">Shipping policy</Link></div><small>© 2026 UTILIPAY HUB (OPC) PRIVATE LIMITED</small></footer><a className="nsx-whatsapp" href={`https://wa.me/${settings.whatsapp}?text=Hi%2C%20I%27d%20like%20to%20discuss%20a%20digital%20project%20with%20UTILIPAY%20HUB.`} target="_blank" rel="noopener noreferrer"><span>●</span> WhatsApp</a></>}
